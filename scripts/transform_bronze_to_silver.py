...
def main(execution_date):
    ...
    spark = (
        SparkSession.builder.appName("BronzeToSilver-Deduplication")
    ...
    df_cleaned = (
        df_raw.filter(col("Invoice").isNotNull())
        .withColumn(
        ...
        .withColumn(
            "revenue",
            col("Quantity").cast(DoubleType()) * col("Price").cast(DoubleType()),
        )
        .select(
    ...
    df_cleaned.write.format("delta").mode("append").option("mergeSchema", "true").save(
        silver_path
    )
